...
function cdspCreateService() {
    ...
    $chk = db()->prepare("SELECT 1 FROM services WHERE service_name=:n AND parent_service_id=:p AND deleted_at IS NULL LIMIT 1");
    $chk->execute([':n' => $name, ':p' => $pid]);
    if ($chk->fetchColumn()) error('A service with that name already exists.', 409);
    $chk2 = db()->prepare("SELECT 1 FROM services WHERE service_code=:c AND deleted_at IS NULL LIMIT 1");
    $chk2->execute([':c' => $code]);
    if ($chk2->fetchColumn()) {
    ...
    $row = $s->fetch();
    logActivity(currentUserId(), 'Create Service', 'cdsp-maintenance', "Created service: {$row['service_name']} ({$row['service_code']})");
...
function cdspDeleteService($id) {
    ...
    $pid = cdspParentServiceId();
    $chk = db()->prepare("SELECT service_name FROM services WHERE service_id=:id AND parent_service_id=:pid AND deleted_at IS NULL LIMIT 1");
    $chk->execute([':id' => $id, ':pid' => $pid]);
    $name = $chk->fetchColumn();
    if ($name === false) error('Service not found or not a CDSP service.', 404);
    $cntS = db()->prepare("SELECT COUNT(*) FROM cdsp_activities WHERE service_id=:id AND deleted_at IS NULL");
    $cntS->execute([':id' => $id]);
    $cnt = (int)$cntS->fetchColumn();
    if ($cnt > 0) error("Cannot delete: $cnt activit" . ($cnt === 1 ? 'y uses' : 'ies use') . ' this service.', 409);
    db()->prepare("UPDATE services SET deleted_at=now(), deleted_by=:uid WHERE service_id=:id")->execute([':uid' => currentUserId(), ':id' => $id]);
    logActivity(currentUserId(), 'Delete Service', 'cdsp-maintenance', "Deleted service: {$name}");
...
function cdspCreateActivity() {
    ...
    logActivity(currentUserId(), 'Create Activity', 'cdsp-maintenance', "Created activity: " . trim($d['title']));
...
function cdspUpdateActivity($id) {
    ...
    logActivity(currentUserId(), 'Update Activity', 'cdsp-maintenance', "Updated activity: " . trim($d['title']));
...
function cdspUpdateActivityStatus($id) {
    ...
    logActivity(currentUserId(), 'Update Activity Status', 'cdsp-maintenance', "Set activity '{$title->fetchColumn()}' status to {$status}");
...
function cdspDeleteActivity($id) {
    ...
    $titleS = db()->prepare("SELECT activity_title FROM cdsp_activities WHERE activity_id=:id AND deleted_at IS NULL");
    $titleS->execute([':id' => $id]);
    $title = $titleS->fetchColumn();
    if ($title === false) error('Activity not found.', 404);

    db()->prepare("UPDATE cdsp_activities SET deleted_at=now(), deleted_by=:uid WHERE activity_id=:id")->execute([':uid' => currentUserId(), ':id' => $id]);
    logActivity(currentUserId(), 'Delete Activity', 'cdsp-maintenance', "Deleted activity: {$title}");
...
function cdspUpdateProfile($id) {
    ...
    logActivity($uid, 'Update Profile', 'cdsp', "Updated applicant: " . trim($d['lastName']) . ", " . trim($d['firstName']));
...
function cdspDeleteProfile($id) {
    ...
    logActivity($uid, 'Delete Profile', 'cdsp', "Moved to recycle bin: " . ($nameRow ? $nameRow['last_name'] . ', ' . $nameRow['first_name'] : "#{$bid}"));
...
// recordType -> [table, primary-key column].
function cdspRecycleMap() {
    return [
        'cdspApplicant' => ['beneficiaries', 'beneficiary_id'],
        'cdspService'   => ['services', 'service_id'],
        'cdspActivity'  => ['cdsp_activities', 'activity_id'],
    ];
}
...
function cdspListDeleted() {
    ...
    $svcS = db()->prepare(
        "SELECT sv.service_id AS id, sv.service_name AS name, sv.deleted_at, u.username AS deleted_by
         FROM services sv
         LEFT JOIN users u ON u.user_id = sv.deleted_by
         WHERE sv.parent_service_id = :pid AND sv.deleted_at IS NOT NULL"
    );
    $svcS->execute([':pid' => $pid]);
    foreach ($svcS->fetchAll() as $r) {
        $items[] = [
            'recordType'  => 'cdspService',
            'id'          => (int) $r['id'],
            'name'        => $r['name'],
            'module'      => 'CDSP Services',
            'description' => 'CDSP sub-service',
            'deletedBy'   => $r['deleted_by'] ?? '',
            'deletedAt'   => $r['deleted_at'],
        ];
    }
    ...
    $actS = db()->prepare(
        "SELECT ca.activity_id AS id, ca.activity_title AS name, ca.deleted_at, u.username AS deleted_by
         FROM cdsp_activities ca
         LEFT JOIN users u ON u.user_id = ca.deleted_by
         WHERE ca.deleted_at IS NOT NULL"
    );
    $actS->execute();
    foreach ($actS->fetchAll() as $r) {
        $items[] = [
            'recordType'  => 'cdspActivity',
            'id'          => (int) $r['id'],
            'name'        => $r['name'],
            'module'      => 'CDSP Activities',
            'description' => 'CDSP activity',
            'deletedBy'   => $r['deleted_by'] ?? '',
            'deletedAt'   => $r['deleted_at'],
        ];
    }
...
function cdspPurgeRecord() {
    ...
    if ($type === 'cdspApplicant') {
        ...
        // Services/activities were already blocked from being soft-deleted while
        // still referenced (see cdspDeleteService/cdspDeleteActivity's in-use
        // guards), so a plain row delete here is safe — no cascade needed.
