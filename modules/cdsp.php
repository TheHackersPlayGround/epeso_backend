...
function cdspCreateService() {
    ...
    $chk2->execute([':c' => $code]);
    if ($chk2->fetchColumn()) error('Service code "' . $code . '" is already in use. Try a different code.', 409);
...
function cdspUpdateActivity($id) {
    ...
    $status = in_array($d['status'] ?? '', $valid, true) ? $d['status'] : 'Planned';
    db()->prepare(
        "UPDATE cdsp_activities SET activity_title=:t,description=:desc,activity_date=:date,location=:loc,facilitator=:fac,participant_count=:pc,status=:st,counselor=:c,session_duration=:dur,updated_at=now() WHERE activity_id=:id"
...
function cdspBuildProfile($bid) {
    $s = db()->prepare(
        "SELECT b.*, bgy.barangay_name, c.city_name, p.province_name, r.region_name,
                ...
                bs.date_applied, bs.status AS bs_status, sv.service_name AS service_availed
         FROM beneficiaries b
         JOIN beneficiary_services bs ON bs.beneficiary_id = b.beneficiary_id
         JOIN services sv ON sv.service_id = bs.service_id
...
function cdspCreateProfile() {
    ...
    try {
        ...
        $s2 = $pdo->prepare("INSERT INTO beneficiary_services(beneficiary_id,service_id,status,date_applied,received_by) VALUES(:bid,:sid,'Active',:date,:uid) RETURNING beneficiary_service_id");
        $s2->execute([':bid'=>$bid,':sid'=>$serviceId,':date'=>cdspDate($d['dateApplicationReceived']??'')??date('Y-m-d'),':uid'=>$uid]);
        ...
        $pdo->prepare("INSERT INTO cdsp_profiles(beneficiary_service_id,course_program,strand,year_level,year_graduated,employment_status,current_occupation,date_received,received_by,remarks,status) VALUES(:bsid,:course,:strand,:ylvl,:yr,:empst,:occ,:drec,:rby,:rmk,'Active')")
            ->execute([':bsid'=>$bsId,':course'=>cdspNullStr($d['course']??''),':strand'=>cdspNullStr($d['strand']??''),':ylvl'=>cdspNullStr($d['yearLevel']??''),':yr'=>cdspYearOrNull($d['yearGraduated']??''),':empst'=>cdspNullStr($d['employmentStatus']??''),':occ'=>cdspNullStr($d['currentOccupation']??''),':drec'=>cdspDate($d['dateApplicationReceived']??''),':rby'=>cdspNullStr($d['receivedBy']??''),':rmk'=>cdspNullStr($d['remarks']??'')]);
...
function cdspUpdateProfile($id) {
    ...
    try {
        ...
        $ex->execute([':id'=>$bsId]);
        if ($ex->fetchColumn()) {
            $pdo->prepare("UPDATE cdsp_profiles SET course_program=:course,strand=:strand,year_level=:ylvl,year_graduated=:yr,employment_status=:empst,current_occupation=:occ,date_received=:drec,received_by=:rby,remarks=:rmk,updated_at=now() WHERE beneficiary_service_id=:bsid")
                ->execute([':course'=>cdspNullStr($d['course']??''),':strand'=>cdspNullStr($d['strand']??''),':ylvl'=>cdspNullStr($d['yearLevel']??''),':yr'=>cdspYearOrNull($d['yearGraduated']??''),':empst'=>cdspNullStr($d['employmentStatus']??''),':occ'=>cdspNullStr($d['currentOccupation']??''),':drec'=>cdspDate($d['dateApplicationReceived']??''),':rby'=>cdspNullStr($d['receivedBy']??''),':rmk'=>cdspNullStr($d['remarks']??''),':bsid'=>$bsId]);
        } else {
            $pdo->prepare("INSERT INTO cdsp_profiles(beneficiary_service_id,course_program,strand,year_level,year_graduated,employment_status,current_occupation,date_received,received_by,remarks,status) VALUES(:bsid,:course,:strand,:ylvl,:yr,:empst,:occ,:drec,:rby,:rmk,'Active')")
                ->execute([':bsid'=>$bsId,':course'=>cdspNullStr($d['course']??''),':strand'=>cdspNullStr($d['strand']??''),':ylvl'=>cdspNullStr($d['yearLevel']??''),':yr'=>cdspYearOrNull($d['yearGraduated']??''),':empst'=>cdspNullStr($d['employmentStatus']??''),':occ'=>cdspNullStr($d['currentOccupation']??''),':drec'=>cdspDate($d['dateApplicationReceived']??''),':rby'=>cdspNullStr($d['receivedBy']??''),':rmk'=>cdspNullStr($d['remarks']??'')]);
        }
