...
// whoever reads it later; the applicant's actual name (or batch/activity
// name) does. Must be looked up BEFORE the record is hard-deleted.
function stRecordName($type, $id) {
    if ($type === 'skillsTrainingApplicant') {
        ...
        $s = db()->prepare("SELECT batch_name FROM skills_training_batches WHERE batch_id = :id");
    } else {
        $s = db()->prepare("SELECT activity_title FROM skills_training_activities WHERE activity_id = :id");
    }
    $s->execute([':id' => $id]);
    $name = $s->fetchColumn();
...
function stRestoreRecord() {
    ...
    logActivity(currentUserId(), 'Restore Record', 'skills', "Restored \"{$name}\" from recycle bin");
    json(['status' => 'ok', 'message' => 'Record restored.']);
}

// Single place that knows how to actually hard-delete each recordType --
// shared by stPurgeRecord() (one record, explicit admin action) and
// stPurgeExpired() (bulk, automatic 30-day retention purge) so there's
// never a second, drifting copy of this dispatch.
function stHardDeleteByType($type, $id) {
    if ($type === 'skillsTrainingApplicant') {
...
function stPurgeRecord() {
    ...
    $name = stRecordName($type, $id);
    stHardDeleteByType($type, $id);
    logActivity(currentUserId(), 'Purge Record', 'skills', "Permanently deleted \"{$name}\"");
...
// stListDeleted() so simply viewing the recycle bin enforces the "N days
// left" countdown the UI already shows -- that display was cosmetic only
// until this existed.
function stPurgeExpired() {
    $uid = currentUserId();

    // Applicants: beneficiaries.deleted_at is a SHARED column (one row per
    // person, not per-service) -- an unscoped query here would let Skills
    // Training's sweep pick up and hard-delete someone who actually belongs
    // to a different module. Scoped exactly like stListDeleted().
    $appS = db()->prepare(
        "SELECT b.beneficiary_id AS id
         FROM beneficiaries b
    ...
    $appS->execute([':sid' => stServiceId(), ':days' => RECYCLE_BIN_RETENTION_DAYS]);
    foreach ($appS->fetchAll(PDO::FETCH_COLUMN) as $id) {
        $id = (int) $id;
        $name = stRecordName('skillsTrainingApplicant', $id);
        stHardDeleteByType('skillsTrainingApplicant', $id);
        logActivity($uid, 'Auto-Purge Record', 'skills', "Automatically purged \"{$name}\" after " . RECYCLE_BIN_RETENTION_DAYS . "-day recycle bin retention");
    }

    // Batches and Activities: standalone module-specific tables, no
    // cross-module ambiguity -- safe to use the plain recycle-map-driven query.
    foreach (['skillsTrainingBatch', 'skillsTrainingActivity'] as $type) {
        [$table, $pk] = stRecycleMap()[$type];
        $s = db()->prepare(
